  ...
  return `
    <div class="product-card" id="product-${product.slug}" onclick="handleProductCardClick('${product.slug}', '${product._id || ''}')" style="cursor: pointer;">
      <div class="product-image">
        <img src="${resolveImage(defaultFlavor.image)}" alt="${product.name}" loading="lazy">
      ...
      <div class="product-info">
        ...
        <div class="product-rating">
          ...
          <span class="rating-count">(${product.numReviews || product.reviews || 0} reviews)</span>
        </div>

        ${showViewers ? `
          <div style="display: flex; align-items: center; gap: 6px; margin-bottom: 15px; font-size: 12px; color: var(--text-muted);">
            <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="var(--accent)" stroke-width="2" stroke-linecap="round"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg>
            <span><strong style="color: var(--accent);" id="viewers-${product.slug}">${viewersCount}</strong> people viewing this right now</span>
          </div>
        ` : ''}

        <button class="btn-add-cart" style="width: 100%; background: var(--accent); justify-content: center; text-transform: uppercase; font-weight: 800; letter-spacing: 1px;">
          ⚡ View Details
        </button>
      </div>
    </div>
  ...
  return `
    <div class="product-card combo-card" id="combo-${combo.comboSlug}" onclick="handleComboCardClick('${combo.comboSlug}')" style="cursor: pointer;">
      <div class="product-image" style="background: #000; padding: 0;">
        <img src="${resolveImage(imgUrl)}" alt="${combo.comboName}" loading="lazy" style="height: 100%; width: 100%; object-fit: cover;">
      ...
      <div class="product-info">
        ...
        <button onclick="event.stopPropagation(); handleComboCardClick('${combo.comboSlug}')" class="btn-primary btn-combo" style="width: 100%; justify-content: center; padding: 12px; font-size: 14px; background: linear-gradient(135deg, #9b59b6, #8e44ad);">⚡ View Details</button>
      </div>
    </div>
  `;
}
...
function handleProductCardClick(slug, productId) {
  ...
  // Save view event to sessionStorage to process it on next page load (avoids navigation cancellation)
  sessionStorage.setItem('lr_pending_view_event', JSON.stringify({ productId, productName, source: "card_click", price }));
...
function handleComboCardClick(slug) {
  ...
  const comboName = combo ? combo.comboName : "Unknown Combo";
  const price = combo ? combo.finalPrice : 0;
  
  // Save combo view event to sessionStorage to process it on product page load
  sessionStorage.setItem('lr_pending_view_event', JSON.stringify({ productId: combo?._id || "combo", productName: comboName, source: "combo_click", price, isCombo: true }));
  ...
  resultsEl.innerHTML = filtered.map(p => {
    ...
    return `
      <a href="product.html?slug=${p.slug}" class="search-result-item">
        <img src="${resolveImage(firstFlavor.image)}" alt="${p.name}">
        <div>
...
function updateCartUI() {
  ...
  const itemsHTML = cart.map(item => {
    ...
    <div style="display:flex; gap:12px; padding:16px 0; border-bottom:1px solid var(--border);">
      <img src="${resolveImage(item.image)}" style="width:60px;height:60px;object-fit:contain;background:#0e0e0e;border-radius:6px;padding:4px;" alt="${item.name}">
      <div style="flex:1;">
...
function startCartCheckout() {
  ...
  if (typeof window.gtag === "function") {
    window.gtag("event", "begin_checkout", {
      ...
      items: cart.map(i => ({
        item_id: i.productId || i.comboId || i.key,
        item_name: i.name,
        price: i.price,
        quantity: i.qty
      }))
...
// ===== GLOBAL FORCE REFRESH POLLING =====
let currentSiteVersion = null;
async function checkSiteVersion() {
  try {
    const res = await fetch(`${APP_CONFIG.apiUrl}/settings/version`, { cache: 'no-store' });
    const data = await res.json();
    if (data.success) {
      const serverVersion = Number(data.version);
      if (currentSiteVersion === null) {
        currentSiteVersion = serverVersion;
      } else if (serverVersion > currentSiteVersion) {
...
setInterval(checkSiteVersion, 15000);
checkSiteVersion();
  ...
  try {
    if (isCombo) {
      ...
      if (data.success) {
        ...
        if (combo) {
          ...
          // Process pending view event or track as default combo view
          const pendingView = sessionStorage.getItem('lr_pending_view_event');
          if (pendingView) {
            try {
              const { productId, productName, source, price } = JSON.parse(pendingView);
              trackProductViewEvent(productId, productName, source, price);
            } catch (_) {}
            sessionStorage.removeItem('lr_pending_view_event');
          } else {
            trackProductViewEvent(combo._id, combo.comboName, "combo_page", combo.finalPrice);
          }
          return;
        }
      }
    ...
    if (data.success) {
      ...
      if (pendingView) {
        try {
          const { productId, productName, source, price } = JSON.parse(pendingView);
          if (productId === currentProductData._id) {
            trackProductViewEvent(productId, productName, source, price);
          } else {
  ...
  const galleryHTML = currentGalleryImages.length > 1 ? `
    <div class="thumbnail-gallery">
      ${currentGalleryImages.map((img, idx) => `
        <img src="${resolveImage(img)}" class="thumbnail-img" 
    ...
    <div class="modal-grid" style="position: relative; background: var(--bg-secondary); border-radius: 12px; border: 1px solid var(--border); box-shadow: 0 10px 40px rgba(0,0,0,0.5);">
      ...
      <div class="modal-image-col">
        <div class="main-image-wrapper" onmousemove="handleImageZoom(event, this)" onmouseleave="resetImageZoom(this)" ontouchstart="handleTouchStart(event)" ontouchend="handleTouchEnd(event)" style="position: relative;">
          ${galleryArrows}
          <img id="mainModalImage" src="${resolveImage(flavor.image)}" alt="${product.name}" onclick="openImageLightbox(this.src)" style="cursor: zoom-in;" title="Click to zoom">
        </div>
        ${galleryHTML}
        ...
        <div class="modal-tab-content active" id="tab-desc">
          ...
          ${product.isCombo ? `
            <div style="margin-top: 30px; border-top: 1px solid var(--border); padding-top: 25px;">
              <h4 style="font-family: var(--font-heading); color: #9b59b6; text-transform: uppercase; margin-bottom: 15px; display: flex; align-items: center; gap: 8px;">
                ...
                Stack Details / What's Included
              </h4>
              <div style="display: grid; gap: 12px;">
                ${product.products.map(p => `
                  <div style="background: rgba(255,255,255,0.03); border: 1px solid var(--border); border-radius: 8px; padding: 12px; display: flex; gap: 12px; align-items: center;">
                    <img src="${resolveImage(p.image || 'images/logo.png')}" style="width: 50px; height: 50px; object-fit: contain; background: #000; border-radius: 4px; padding: 4px;">
                    <div>
                      <div style="font-weight: 600; font-size: 14px;">${p.name}</div>
                      <div style="font-size: 12px; color: var(--text-muted);">Quantity: ${p.quantity || 1}</div>
                    </div>
                  </div>
                `).join('')}
              </div>
...
function setGalleryImage(index) {
  ...
  document.getElementById('mainModalImage').src = resolveImage(currentGalleryImages[index]);
